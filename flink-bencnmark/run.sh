...
QUERY_CLASS="org.example.flink.yahoo.AdvertisingTopologyFlinkWindows"
...
if [ $1 == "start" ]; then
  ...
  $FLINK run -d -m 127.0.0.1:8081 --class $QUERY_CLASS $JAR_PATH --config conf/benchmarkConf.yaml
fi
